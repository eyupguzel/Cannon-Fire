...
public class SnowController : MonoBehaviour
{
    ...
    void UpdateAll(){
        ...
        snowShape.radius = 1f * Mathf.Clamp(windIntensity, 0.4f, 1f) * masterIntensity;
        snowForce.x = new ParticleSystem.MinMaxCurve(-9f * windIntensity, -3-14f * windIntensity);
        windEmission.rate = 0f * masterIntensity * (windIntensity + fogIntensity);
        windMain.startLifetime = 2f + 6f * (1f - windIntensity);
        windMain.startSpeed = new ParticleSystem.MinMaxCurve(1f * windIntensity, 2 * windIntensity);
